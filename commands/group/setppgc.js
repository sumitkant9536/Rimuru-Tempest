...
module.exports = {
    ...
    noLimit: true,
    start: async(rimuru, m, { mime, quoted, text, prefix, command }) => {
        if (/image/.test(mime)) {
            let media = await rimuru.downloadAndSaveMediaMessage(quoted)
            if (text.toLowerCase() === "original") {
                ...
                await rimuru.updateProfilePicture(m.from, { url: media })
                .then( res => {
                    m.reply("Sukses")
                    fs.unlinkSync(media)
                }).catch(() => m.reply('err'))
