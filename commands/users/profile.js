...
module.exports = {
	...
	noLimit: true,
    start: async(rimuru, m, {isPremium, isOwner}) => {
		let statuses
		try {
			statuses = await rimuru.fetchStatus(m.sender);
		} catch {
			statuses = "Nothing.."
		}
		...
		try {
			var pp = await rimuru.profilePictureUrl(m.sender, "image");
		} catch {
		...
		caption += `│\n`
		caption += `└───────⭓\n`
		rimuru.sendFile(m.from, pp, "", m, { caption })
    }
}