...
const connect = async () => {
    ...
    const rimuru = new WAConnection(global.opts["legacy"] ? makeWALegacySocket(connOptions) : makeWASocket(connOptions))
    ...
    global.Store = Store.bind(rimuru)

    rimuru.ev.on("creds.update", saveState)

    rimuru.ev.on("connection.update", async(update) => {
        if (update.connection == "open" && rimuru.type == "legacy") {
            rimuru.user = {
                id: rimuru.state.legacy.user.id,
                jid: rimuru.state.legacy.user.id,
                name: rimuru.state.legacy.user.name
            }
        }
        ...
        if (connection == "close") {
            ...
            if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete Session and Scan Again`); rimuru.logout(); }
            ...
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); rimuru.logout(); }
            ...
            else rimuru.end(`Unknown DisconnectReason: ${reason}|${connection}`)
        }
    })

    // Welcome
    rimuru.ev.on("group-participants.update", async (m) => {
		Welcome(rimuru, m);
	});

    rimuru.ev.on("messages.upsert", async (chatUpdate) => {
        m = serialize(rimuru, chatUpdate.messages[0])

        if (!m.message) return
        if (m.key && m.key.remoteJid == "status@broadcast") return
        if (m.key.id.startsWith("BAE5") && m.key.id.length == 16) return
        require("./rimuru")(rimuru, m, Commands, chatUpdate)
    })

    if (rimuru.user && rimuru.user?.id) rimuru.user.jid = jidNormalizedUser(rimuru.user?.id)
    rimuru.logger = (rimuru.type == "legacy") ? DEFAULT_LEGACY_CONNECTION_CONFIG.logger.child({ }) : DEFAULT_CONNECTION_CONFIG.logger.child({ })
}
