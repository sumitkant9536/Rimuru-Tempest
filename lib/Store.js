...
function bind(rimuru) {
    ...
    rimuru.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    rimuru.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    rimuru.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    rimuru.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   rimuru.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    rimuru.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    rimuru.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    rimuru.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    rimuru.ev.on('chats.update', updates => {
    ...
    rimuru.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    rimuru.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**rimuru.ev.on('messages.update', updates => {
    ...
    rimuru.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    rimuru.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    rimuru.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (rimuru === null || rimuru === void 0 ? void 0 : rimuru.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (rimuru === null || rimuru === void 0 ? void 0 : rimuru.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (rimuru === null || rimuru === void 0 ? void 0 : rimuru.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return rimuru === null || rimuru === void 0 ? void 0 : rimuru.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (rimuru === null || rimuru === void 0 ? void 0 : rimuru.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (rimuru === null || rimuru === void 0 ? void 0 : rimuru.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (rimuru === null || rimuru === void 0 ? void 0 : rimuru.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (rimuru === null || rimuru === void 0 ? void 0 : rimuru.messageInfo(remoteJid, id))
                if (msg) {
